...
translations = {
    ...
    "buy_no_seed": {
        "uk": f"{emoji_dict['error']} У вас закінчилося пакет насіння {{name}}.",
        "ru": f"{emoji_dict['error']} у вас закончилось пакет семя {{name}}.",
        "en": f"{emoji_dict['error']} You have run out of seed packet {{name}}.",
    },
    "not_enough_money": {
...
def main_menu_keyboard(lang):
    ...
    builder.add(types.KeyboardButton(text="/top"))
    builder.adjust(2)
